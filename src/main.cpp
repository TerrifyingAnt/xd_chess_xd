...
// последний совершенный ход в игре
ChessMove lastMove;
...
// ход, который совершает пользователь
ChessMove chessMoveFrom;

// вектор возможных ходов
std::vector<unsigned char> figurePossibleMoves;

// хранится актуальное поле с герконов, то есть сюда записывается в реальном времени инфа
bool gerkonActualFieldData[64];
// последнее состояние доски, то есть сюда записываются прям ходы с доски
...
// функция в котором происходит связь с апи
void computerMove(ChessBoard& board, int depth, int maxSteps);

// подсветка доски исходя из фигур, которые записаны в lastStateGerkonFieldData
void ledForFigures(ChessBoard& board);

// функция по началу игры
void playGame();

// создание актуального поля игры
void generateActualFieldFromGerkons();

// подсветка доски исходя из текущего состояния доски
void ledBoard();

// функция получения информации с герконов
void shiftRegisters();

// функция показания возможных ходов (не работает пока, там ошибка по индексу (index out of bound которая))
void showPossibleMoves(ChessMove chessMoveFrom);

// функция, которая проигрывает игру по массиву ходов
void startDemoGame();

// функция калибровки доски
void calibrate();

// функция подсветки ленты
void rainbowCycle(uint8_t wait);

// функция для подсветки ленты
uint32_t Wheel(byte WheelPos);

// подсветка крутиться 5 цветов ретровейвных
void ledRetroWave();

// подсветка дыхание в режиме ожидания
void ledBreathIdle();
...
int userMove(ChessBoard& tempBoard) {
  while (true) {
    ...
    // в цикле сравниваем, если последнее состояние отличается от текущего
    // с герконов (пользователь поднял фигуру), то смотрим
    for (int i = 0; i < 8; i++) {
      for (int j = 0; j < 8; j++) {
        // первое изменение и есть место, откуда пользователь пошел
        if (gerkonActualFieldData[i * 8 + j] != lastStateGerkonFieldData[i * 8 + j] &&
         from == false && lastStateGerkonFieldData[i * 8 + j] == true) {
          from = true;
          fromInt = i * 8 + j;
          Serial.print(" from: ");
    ...
    if (toInt == -1) {
      // TODO(me) - рефакторинг - зачем еще один иф
      if (from) {
...
void computerMove(ChessBoard& tempBoard) {
  ...
  while (!moveWasMade) {
    ...
    if (!move1.equals(move2)) {
      ...
      tempBoard.kingMoves = tempBoard.getAllPossibleMovesWithKingExchange();
  ...
  if (chessMoveFrom.from != 0) {
    ...
    if (possibleMovesCount > 0) {
      for (int j = 0; j < 4; j++) {
        ...
        mtrx.update();
        if (j == 3) {
          // задержка для считывания регистров == 100,
          // таким образом не будет задержек при прокручивании анимации возможного хода
          delay(400);
        } else {
          delay(500);
        }
...
void startDemoGame() {
  String chessMovesStringList[8] = {"e2e4", "d7d5", "f1d3", "e7e6", "b1c3", "d8g5", "h2h3", "g5e3"};
  for (int i = 0; i < sizeof(chessMovesStringList); i++) {
    String move = chessMovesStringList[i];
    char buffer[6];
    buffer[0] = move[0];
    buffer[1] = move[1];
    buffer[2] = ' ';
    ...
    buffer[4] = move[3];
    buffer[5] = '\0';
    ChessMove chessMove = ChessMove(buffer);
    board.performMove(chessMove);
    delay(5000);
    Serial.print("XD");
    ledBoard();
  }
